...
import {isMockKerberosConfig, isTestAuthServiceEnabled, validateTestKerberosTicket} from '@/services/server/test-auth-service';
...
export async function initializeKerberos(config) {
    ...
    if (isMockKerberosConfig(config)) {
        kerberosService = {test: true};
...
export async function validateKerberosTicket(ticket) {
    if (isTestAuthServiceEnabled() && ticket?.startsWith('test-ticket:')) {
        return validateTestKerberosTicket(ticket);
    }
