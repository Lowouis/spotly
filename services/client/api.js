...
function getApiEndpoint() {
    ...
    return endpoint && endpoint !== 'undefined' ? endpoint : '';
}
...
export const fetchIP = async () => {
    try {
        const response = await fetch(getURL('/api/public/client-ip'), {
            headers: {
...
export const updateEntry = async ({id, moderate, returned = false}) => {
    const response = await fetch(getURL(`/api/entry/${id}`), {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
...
export const checkIPAuthorization = async (ip) => {
    try {
        const response = await fetch(getURL(`/api/authorized-location/check/${ip}`));
